...
function RedefinirSenhaContent() {
  ...
  const tokenQuery = useValidarTokenRecuperacao(token);
  const redefinirSenha = useRedefinirSenha();

  useEffect(() => {
    if (redefinirSenha.isSuccess) {
    ...
    return undefined;
  }, [redefinirSenha.isSuccess, router]);
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    if (!token || !tokenQuery.data?.valid) {
    ...
    redefinirSenha.mutate({ token, novaSenha, confirmarSenha });
  };

  const carregando = redefinirSenha.isPending;
  const mensagemErro =
    erro ||
    (redefinirSenha.error instanceof Error ? redefinirSenha.error.message : "");
  const tokenInvalido =
    !token ||
    tokenQuery.isError ||
    (!tokenQuery.isLoading && tokenQuery.data?.valid === false);
      ...
      <div className="min-h-screen bg-gradient-to-br from-[#F3EAF8] to-[#E1D4F0] flex flex-col items-center justify-center p-4">
        ...
        <div className="w-full max-w-md bg-white/80 backdrop-blur-sm rounded-2xl shadow-xl p-8 text-center">
          ...
          <Link href="/recuperar-senha">
            <button
              type="button"
              className="bg-[#9F64AF] text-white py-2 px-4 rounded-lg hover:bg-[#8B509B] transition-all duration-300"
            >
        ...
        <form onSubmit={handleSubmit} className="space-y-4">
          <div>
            ...
            <p className="text-xs text-gray-400 mt-1">
              A senha deve ter no mínimo 8 caracteres, conter números, letras
          ...
          <div className="text-center mt-3">
            <Link href="/">
              <button
                type="button"
                className="text-[#9F64AF] text-sm hover:underline transition-all duration-200"
              >
                Voltar para o login
              </button>
            </Link>
