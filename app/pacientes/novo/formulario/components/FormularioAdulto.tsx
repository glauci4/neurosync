...
export default function FormularioAdulto({ onSubmit, isPending, error: apiError, onClose }: FormularioAdultoProps) {
    const pacienteRef = useRef<DadosPacienteRef>(null);
    const [formData, setFormData] = useState({
        tipo: 'adulto', nome: '', data_nascimento: '', genero: '', raca_etnia: '', cpf: '',
    ...
    const validarIdade = (): boolean => {
        ...
        if (idade < 18) { setErroIdade('Paciente adulto deve ter 18 anos ou mais'); return false; }
    ...
    const validarContato = (): boolean => {
        if (contatoEmergencia.nome) {
            ...
            if (!telefoneNumeros || telefoneNumeros.length < 10) {
                setErroContato('Telefone do contato de emergência é obrigatório quando o nome é informado'); return false;
            }
            const telefonePaciente = formData.telefone.replace(/\D/g, '');
            if (telefonePaciente && telefoneNumeros === telefonePaciente) {
                setErroContato('Telefone do contato de emergência não pode ser igual ao telefone do paciente'); return false;
            }
        }
        setErroContato(''); return true;
    };
