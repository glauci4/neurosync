...
function TipoPacienteToggle({
  tipo,
  onChange,
}: {
  tipo: "adulto" | "menor";
  onChange: (t: typeof tipo) => void;
}) {
  return (
...
export default function CadastroPacienteModal({
  ...
  useEffect(() => {
    ...
    const handleKeyDown = (evento: KeyboardEvent) => {
      if (evento.key === "Escape" && !isPending) {
        reset();
        onClose();
      }
    };
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center p-4">
      ...
      >
        ...
        <TipoPacienteToggle tipo={tipo} onChange={handleTipoChange} />
