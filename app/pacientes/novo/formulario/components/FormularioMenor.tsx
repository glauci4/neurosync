...
import DadosPaciente, { DadosPacienteRef } from './DadosPaciente';
...
export default function FormularioMenor({ onSubmit, isPending, error: apiError, onClose }: FormularioMenorProps) {
    const pacienteRef = useRef<DadosPacienteRef>(null);
    const [formData, setFormData] = useState({
        tipo: 'menor', nome: '', data_nascimento: '', genero: '', raca_etnia: '', cpf: '',
        telefone: '', telefone_alternativo: '', email: '', possui_deficiencia: false,
        descricao_deficiencia: '', renda_familiar: '', possui_cadastro_unico: false,
        cep: '', rua: '', numero: '', complemento: '', bairro: '', cidade: '', estado: '',
        observacoes: '', sem_numero: false,
    });
    const [responsavel, setResponsavel] = useState({
        nome: '', cpf: '', telefone: '', grau_parentesco: '', mesmo_endereco_paciente: true,
        sem_numero: false, cep: '', rua: '', numero: '', complemento: '', bairro: '', cidade: '', estado: '',
    });
    ...
    const copiarEnderecoParaResponsavel = () => {
        setResponsavel(prev => ({
            ...
            cidade: formData.cidade || '', estado: formData.estado || '', sem_numero: formData.sem_numero || false,
        }));
    };
    useEffect(() => {
        if (responsavel.mesmo_endereco_paciente) copiarEnderecoParaResponsavel();
    }, [formData.cep, formData.rua, formData.numero, formData.complemento, formData.bairro, formData.cidade, formData.estado, formData.sem_numero, responsavel.mesmo_endereco_paciente]);
    ...
    const handleMesmoEnderecoChange = (checked: boolean) => {
        if (checked) {
            ...
            setResponsavel(prev => ({ ...prev, mesmo_endereco_paciente: false, sem_numero: false, cep: '', rua: '', numero: '', complemento: '', bairro: '', cidade: '', estado: '' }));
        }
    };
    ...
    const validarResponsavel = (): boolean => {
        ...
        if (!responsavel.nome || responsavel.nome.trim().length < 3) { novosErros.nome = 'Nome do responsável é obrigatório'; valido = false; }
        const cpfNumeros = responsavel.cpf.replace(/\D/g, '');
        if (!cpfNumeros) { novosErros.cpf = 'CPF do responsável é obrigatório'; valido = false; }
        else if (cpfNumeros.length !== 11) { novosErros.cpf = 'CPF deve ter 11 dígitos'; valido = false; }
        const telResp = responsavel.telefone.replace(/\D/g, '');
        if (!telResp || telResp.length < 10) { novosErros.telefone = 'Telefone do responsável é obrigatório'; valido = false; }
        ...
        if (telPaciente && telResponsavel && telPaciente === telResponsavel) {
            novosErros.telefone = 'Telefone do responsável não pode ser igual ao telefone principal do paciente';
            valido = false;
        }
        if (!responsavel.grau_parentesco) { novosErros.grau = 'Grau de parentesco é obrigatório'; valido = false; }
        if (!responsavel.mesmo_endereco_paciente) {
            const cepNumeros = responsavel.cep.replace(/\D/g, '');
            if (!cepNumeros || cepNumeros.length !== 8) { novosErros.cep = 'CEP é obrigatório (8 dígitos)'; valido = false; }
            if (!responsavel.rua) novosErros.rua = 'Rua é obrigatória';
            if (!responsavel.sem_numero && !responsavel.numero) novosErros.numero = 'Número é obrigatório';
            if (!responsavel.bairro) novosErros.bairro = 'Bairro é obrigatório';
            if (!responsavel.cidade) novosErros.cidade = 'Cidade é obrigatória';
            if (!responsavel.estado || responsavel.estado.length !== 2) novosErros.estado = 'Estado (UF) é obrigatório';
            if (novosErros.rua || novosErros.numero || novosErros.bairro || novosErros.cidade || novosErros.estado) valido = false;
        }
    ...
    const validarIdade = (): boolean => {
        ...
        if (idade >= 18) { setErroIdade('Paciente menor deve ter menos de 18 anos'); return false; }
        setErroIdade(''); return true;
    };
    ...
    const validarContato = (): boolean => {
        if (contatoEmergencia.nome) {
            ...
            if (telefonePaciente && telefoneNumeros === telefonePaciente) {
                setErroContato('Telefone do contato de emergência não pode ser igual ao telefone do paciente');
                return false;
            }
        }
        setErroContato('');
    ...
    const handleSubmit = (e: React.FormEvent) => {
        ...
        if (!pacienteValido || !responsavelValido || !idadeValida || !contatoValido) return;
        ...
        const dadosEnvio = {
            ...
            responsavel: {
                nome: responsavel.nome, cpf: responsavel.cpf, telefone: responsavel.telefone,
                ...
                ...(responsavel.mesmo_endereco_paciente ? {} : {
                    ...
                    rua: responsavel.rua, numero: responsavel.numero, complemento: responsavel.complemento,
                    bairro: responsavel.bairro, cidade: responsavel.cidade, estado: responsavel.estado,
                }),
            },
    ...
    return (
        <form onSubmit={handleSubmit} className="bg-white/80 backdrop-blur-sm rounded-2xl shadow-xl p-6 space-y-6">
            ...
            />
            <DadosResponsavel responsavel={responsavel} setResponsavel={setResponsavel} erros={errosResponsavel} onMesmoEnderecoChange={handleMesmoEnderecoChange} />
            {/* A prop erroTelefoneExterno leva a mensagem de erro para o componente ContatoEmergencia */}
            <ContatoEmergencia contato={contatoEmergencia} setContato={setContatoEmergencia} erroTelefoneExterno={erroContato} />
            <div className="flex justify-end gap-3 pt-4 border-t border-gray-100">
                <button type="button" onClick={onClose} className="px-4 py-2 border border-gray-300 rounded-lg text-gray-700 hover:bg-gray-50 transition">Cancelar</button>
                <button type="submit" disabled={isPending} className="px-6 py-2 bg-[#9F64AF] text-white rounded-lg hover:bg-[#8B509B] transition disabled:opacity-50">{isPending ? 'Cadastrando...' : 'Cadastrar Paciente'}</button>
            </div>
        </form>
    );
}