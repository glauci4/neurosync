...
export default function PacientesPage() {
    ...
    const { usuario, carregando: authCarregando, fazerLogout } = useAutenticacao();
    const { isCollapsed } = useSidebar();

    // ESTADOS 
    const [visibilidade, setVisibilidade] = useState<Visibilidade>('ativo');
    const [statusAtendimento, setStatusAtendimento] = useState<StatusAtendimento>('fila_espera');
    ...
    const [pacienteIdDetalhes, setPacienteIdDetalhes] = useState<number | null>(null);
    ...
    const { mutate: atualizarPaciente } = useAtualizarPaciente();
    const { mutate: atualizarStatusAtendimento } = useAtualizarStatusAtendimento();
  ...
  // EFEITO PARA ABRIR O MODAL DE CADASTRO SE HOUVER O PARÂMETRO "cadastrar=1" NA URL (ex: após cadastro rápido ou clique em "Cadastrar novo paciente" na barra de filtros)
    ...
    const handleImportarDados = () => router.push('/pacientes/importar');
    ...
    const handleInativarPaciente = () => {
        if (pacienteIdDetalhes) {
            const nome = pacientes.find(p => p.id === pacienteIdDetalhes)?.nome || 'paciente';
            setModalConfig({
                isOpen: true,
                tipo: 'inativar',
    ...
    const handleIniciarAtendimento = (id: number) => {
        atualizarStatusAtendimento({ id, status: 'em_atendimento' });
        setModalDetalhesAberto(false);
        refetch();
    };
    ...
    const tipoVazio = 
        visibilidade === 'inativo' ? 'inativos' :
        (visibilidade === 'ativo' || visibilidade === 'todos') && statusAtendimento
            ? statusAtendimento
            : undefined;
        ...
        <div className="min-h-screen bg-gradient-to-br from-[#F3EAF8] to-[#E1D4F0]">
            ...
            <div className={marginLeft}>
                <div className="pt-8 px-8 pb-2">
                    <CabecalhoPacientes
                        status={visibilidade === 'ativo' ? 'ativo' : (visibilidade === 'inativo' ? 'inativo' : 'todos')}
                    />
            ...
            <ConfirmarAcaoModal
                ...
                titulo={
                    modalConfig.tipo === 'inativar' ? 'Inativar paciente' :
                    modalConfig.tipo === 'reativar' ? 'Reativar paciente' : 
                    'Excluir paciente'
                }
                mensagem={
